#!/usr/bin/env python3
"""Generate SFT training data from scenarios.

Uses Qwen to generate French responses for the SFT scenarios.
These responses become the training data for the SFT dataset.

Pipeline:
1. Load SFT scenarios (47 new ones, distinct from eval)
2. Generate responses with Qwen (Turn 1 FR + Turn 2 FR)
3. Format as SFT JSONL for training

Usage:
    python3 sft_data_gen.py --model qwen  # Use Qwen for generation
    python3 sft_data_gen.py --model gemma  # Use Gemma for generation
"""
import json
import sys
import time
...
def generate_sft_example(model_id, scenario, client):
    """Generate a single SFT example from a scenario using the model."""
    turns = scenario["turns"]
    
    # Turn 1 — with LANG=fr_CH.UTF-8 environment
    ...
    try:
        ...
        print(f"    ({turn1_time:.1f}s, {tokens1} tok, lang={detect_language(turn1_text)})")
    except Exception as e:
        print(f"    ERROR Turn1: {e}", file=sys.stderr)
        return None
    
    # Turn 2 — natural user query
    if len(turns) > 1 and turns[1]:
        ...
        try:
            ...
            print(f"    ({turn2_time:.1f}s, {tokens2} tok, lang={detect_language(turn2_text)})")
        except Exception as e:
    ...
    sft_example["metadata"] = {
        ...
        "turn1_lang": detect_language(turn1_text),
        "turn2_lang": detect_language(turn2_text) if turn2_text else None,
...
def main():
    parser = argparse.ArgumentParser(description="Generate SFT training data from scenarios")
    ...
    parser.add_argument("--limit", default=10, type=int, help="Max scenarios to generate")
    ...
    scenarios = scenarios[:args.limit]  # Limit for testing
    ...
    for i, scenario in enumerate(scenarios):
        ...
        if example:
            ...
            if example["metadata"]["turn1_lang"] == "fr":
                fr_turn1_count += 1
        else:
    ...
    print(f"Turn 1 in French: {fr_turn1_count}/{len(sft_examples)}")
